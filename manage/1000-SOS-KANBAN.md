...
**Board Last Updated:** 2026-09-06 23:15 by Reason A  
**Session status 2026-09-06 23:15** — Follow-up round after the rename, in the same overall session: the user tried the dark/amber theme and asked for a *light* theme instead (`FREEZE-PLAN-013`, revises `FREEZE-PLAN-011`), found the Medodica font made everything too small to read and asked for a two-column layout plus a 150% default scale (`FREEZE-PLAN-014`), and separately reported they'd found the real root cause of the loudness-jump family of bugs by ear: an already-playing voice gets louder, not just glitches, when an unrelated sibling voice finishes (`FREEZE-BUG-005` - a real design gap in `FREEZE-BUG-001`'s original fix, not a duplicate). All three implemented, built, and bundled this session; `cargo test --workspace` passes 58 tests (50 `prism_dsp`, including a rewritten regression test proving voices no longer recover volume on their own; 8 `prism_plugin`). **Not yet confirmed live** - needs a fresh look in Bespoke Synth covering: the new white/green look, the two-column layout at 150%, and specifically playing an overlapping chord and confirming a surviving note's volume stays flat (never climbs) after a sibling note releases and finishes.
