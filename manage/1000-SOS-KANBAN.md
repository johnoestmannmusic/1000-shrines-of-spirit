...
**Board Last Updated:** 2026-09-07 02:15 by Reason A  
...
**Session status 2026-09-07 02:15** — A batch of four requests in one message, all implemented and committed together: (1) polyphony gain compensation switched from strict 1/n to 1/sqrt(n) - found by ear to be "way too extreme" a per-voice step, now leaning on the existing `soft_limit` safety net (`FREEZE-BUG-002`) instead of algebraic 1/n conservatism (`FREEZE-PLAN-018`); (2) fixed the selected-preset name not surviving a saved host project, even though the underlying param values already did - moved from ephemeral `PrismEditorState` to a persisted `PrismPluginParams` field, same pattern as `sample_path` (`FREEZE-BUG-007`); (3) added MIDI pitch bend with an adjustable semitone range param, which required bumping `MIDI_INPUT` from `Basic` to `MidiCCs` (`FREEZE-PLAN-019`); (4) added a per-voice pan randomizer with center/width params, using a small dependency-free deterministic PRNG (`FREEZE-PLAN-020`). All four DSP-level features are captured in saved presets too, with backward-compatible defaults for presets saved before they existed. `cargo test --workspace` passes 65 tests (55 `prism_dsp`, 10 `prism_plugin`). **Not yet confirmed live** - needs a look in a host covering: chords sounding less abruptly-attenuated per added voice, a preset's name surviving a project save/reload, pitch bend actually bending held notes, and the pan randomizer spreading voices left/right when width is turned up.
