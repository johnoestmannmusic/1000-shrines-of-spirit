...
**Board Last Updated:** 2026-09-07 01:05 by Reason A  
...
**Session status 2026-09-07 01:05** — Continuing the same GUI-follow-up thread: after `FREEZE-PLAN-015`/`FREEZE-PLAN-016`, the user reported the whole editor appeared to open maximized with black space around it in Bespoke Synth - investigated and traced to host window-embedding behavior (see the caveat above), not a plugin bug, no code change made. Confirmed in Reaper the sizing/scaling itself is now correct, but content touches the window edges - added a 30px (at the default scale) margin around the whole editor (`FREEZE-PLAN-017`). Earlier in this same session: the 150% scale still wasn't enough and the 620×560 window guess was wrong in both dimensions (too narrow, too tall) - both bumped further (`FREEZE-PLAN-015`). The user also found a genuinely separate, real bug while testing sample loading: a freshly loaded sample doesn't actually take over playback until Freeze Point is nudged (`FREEZE-BUG-006`) - root-caused to a host-timing race between `editor()` and `initialize()`, not anything related to the render pipeline itself. Also shrank the ADSR graph to match the waveform graph's height (`FREEZE-PLAN-016`). All implemented, built, and bundled; `cargo test --workspace` passes 59 tests (50 `prism_dsp`, 9 `prism_plugin` - one new regression test for the sample-loading race). **Not yet confirmed live** - `FREEZE-PLAN-017`'s margin specifically needs a look in Reaper (the reliable reference host) to confirm 30px reads right; the sample-loading fix and graph-height match still need a Bespoke Synth check too.
