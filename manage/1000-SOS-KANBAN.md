...
**⚠ MOVED OUT OF THIS REPO on 2026-09-07** — SpectralPrism now lives in its own standalone repository at `/home/john/Desktop/00_Projects/00_John Oestmann/SpectralPrism/` (full git history preserved via `git subtree split`), not under `src/plugins/SpectralPrism/` here anymore (that directory has been removed - see the 2026-09-07 21:10 session status below for the full story). **Any future SpectralPrism work should happen in that standalone repo, not this board** - this section is kept for historical reference only. The installed VST3 symlink at `00_JOM-Common/Plugins/VST3/SpectralPrism.vst3` has been re-pointed to the new location already.
...
**Implementation Repository:** `/home/john/Desktop/00_Projects/00_John Oestmann/SpectralPrism/` (moved out of this monorepo 2026-09-07 - was `../src/plugins/SpectralPrism/`)  
...
**Board Last Updated:** 2026-09-07 21:10 by Reason A  
...
**Git workflow note:** the user has asked that commits be left to them from here on - this agent no longer runs `git commit` at all, even after finishing/testing a change. Code and board changes are left staged/ready in the working tree; check `git status`/`git diff` for what's pending rather than assuming recent work is already committed.  
**Session status 2026-09-07 19:10** — Direct follow-up to `FREEZE-PLAN-022`: (1) scrapped the `<name>-parameters.txt` sidecar entirely - the user confirmed the `.json` preset already carries everything needed, making it redundant; (2) added a `BUILD_NUMBER` (YYYYMMDD, stamped at compile time by a new `build.rs`) - saved into every preset (`FREEZE-PLAN-023`) and shown small/dim in the editor's bottom-right corner, so it's possible to tell which plugin version made a given sound or preset; (3) presets keep storing the sample's full absolute path (a real privacy tradeoff for shared presets, discussed and accepted rather than fixed by stripping it, per the user's explicit direction) but now self-heal - if a preset's referenced sample can't be found (e.g. imported on a different machine), the user is prompted via a file dialog to locate it, and a successful relocation is written back into that same preset file so it doesn't have to be re-located again on this machine (`FREEZE-BUG-008`). `cargo test --workspace` passes 68 tests (55 `prism_dsp`, 13 `prism_plugin` - the txt-sidecar tests were removed along with the feature; the relocate-dialog flow itself isn't unit-tested, same limitation as every other `rfd`-based interactive flow in this file). Built and bundled. **Not yet confirmed live** - needs a check: confirm the build number shows correctly in the corner and lands in a freshly-exported preset's JSON, and specifically test the relocate flow by importing a preset whose `sample_path` points somewhere that doesn't exist and confirming the prompt appears and the fix sticks on a second import.
**Session status 2026-09-07 19:35** — Added an adjustable "Loop Length" param (`FREEZE-PLAN-024`), following up on the user noticing the exported WAV "contains a lot of the same content": `render_frozen_loop` no longer auto-derives the frozen loop's length from the source sample (previously clamped 4-8s), it now takes an explicit `loop_length_seconds` the user dials via a new slider (range widened to 0.5-8s, default 4s to match the old floor) - a shorter setting directly shrinks both the exported WAV's file size and the in-memory loop buffer. Saved in presets with a custom serde default (old presets come back at 4s, not a broken 0s). `cargo test --workspace` passes 69 tests (56 `prism_dsp`, 13 `prism_plugin`). Built and bundled. **Not yet confirmed live** - needs a by-ear check that short loop lengths (e.g. 1s) still sound clean rather than obviously repetitive/clicky, and a file-size comparison on an exported WAV at a short vs. long setting.
**Session status 2026-09-07 19:50** — Direct follow-up: the user reported the new short Loop Length setting made the loop's own wraparound click/glitch audibly on a held note, asking specifically for "a crossfade from end to beginning of the loop." Root cause was that `FREEZE-PLAN-024`'s doc comment claiming hop-aligned lengths loop with "no seam discontinuity" was wrong - the frozen resynthesis keeps advancing phase indefinitely, so the wrap point was never actually phase-continuous, just rare enough at the old 4-8s range to go unnoticed. Fixed with an in-place equal-power crossfade at each channel's own seam (`FREEZE-BUG-009`, new `crossfade_loop_seam` + `LOOP_CROSSFADE_SECONDS = 0.05`). `cargo test --workspace` passes 71 tests (58 `prism_dsp`, 13 `prism_plugin`). Built and bundled. **Not yet confirmed live** - needs the exact scenario reported: hold a note at a short Loop Length setting and confirm the click/glitch is gone; report back if 50ms isn't quite enough to fully mask it, since that's just a constant to tune.
**Session status 2026-09-07 20:05** — The crossfade fix above did NOT resolve the reported looping issue (user confirmed still audible), and the user asked directly whether blending the whole loop is even the right approach. It wasn't: replaced the time-domain crossfade entirely with a proper DSP fix (`FREEZE-BUG-009`, updated) - each bin's phase advance is now quantized (`quantize_advance_for_loop`) so it completes a whole number of cycles over the loop, and the overlap-add is now circular (`ola_accumulate_circular`, wraps the last frame's tail back to the start instead of dropping it) - together these make the frozen loop genuinely, mathematically periodic rather than merely smoothed at the seam. No blending needed at all. `cargo test --workspace` passes 74 tests, including a new direct proof that two consecutive loops of phase-locked synthesis are near-bit-identical. Built and bundled. **Confirmed live 2026-09-07** - user tested and confirmed "Works great!"
**Session status 2026-09-07 20:20** — New, unrelated report right after confirming the loop fix: adding voices makes each one noticeably quieter after ~4-5 (with a longer sustain), and they "stay fairly quiet until the number of voices active goes back to 0, then they start loud again." Root cause: the polyphony gain-compensation ratchet (`active_compensation`) only ever got stricter, never recovered until every single voice went silent - a deliberate earlier fix for a different bug (recomputing fresh every block caused an audible swell), but it overcorrected. Fixed (`FREEZE-BUG-010`) by adding a second, much slower recovery path (`GAIN_RECOVERY_MS = 800ms`) alongside the existing instant ratchet-down: compensation now eases back toward full volume within a couple of seconds of the texture thinning out, rather than waiting for total silence, while the safety-critical instant-down direction is untouched. `cargo test --workspace` passes 74 tests. Built and bundled. **Superseded** - user reported it was "still happening, though less"; see below.
**Session status 2026-09-07 20:35** — The slow-recovery fix above wasn't enough, and the user asked directly whether there's a standard synth approach and whether this was over-engineered. It was: three successive attempts at reactive polyphony gain compensation (1/n, then 1/sqrt(n) ratchet, then ratchet + slow recovery) each fixed one problem but shifted the tradeoff, because any gain that reacts to unrelated notes' lifecycle is itself a form of pumping. Removed the whole mechanism (`FREEZE-BUG-011`): voices now sum at a fixed, uncompensated gain always (more notes = more sound, like a real instrument), with the existing `soft_limit` as the sole safety net against an oversized chord (smooth saturation instead of a hard clip, which is expected/normal, not a bug). `cargo test --workspace` passes 74 tests, `cargo build --workspace` clean with no warnings. Built and bundled. **Confirmed live 2026-09-07** - user tested and confirmed "That works so much better thanks."
**Session status 2026-09-07 20:40** — Small follow-up label fix: renamed the "Pan Width" param/UI label to "Random Pan Width" (both the `FloatParam`'s display name in `PrismPluginParams::default()` and the UI `right.label(...)` call in `crates/prism_plugin/src/lib.rs`) - clarifies that it controls the per-voice pan *randomizer*'s spread, not a static stereo-width-style control, distinguishing it from "Pan Center". Parameter ID (`pan_width`) unchanged, so this doesn't affect saved automation/presets - purely a display-name change. `cargo build`/`cargo test --workspace` unaffected (74 tests, no logic touched). Built and bundled.
**Session status 2026-09-07 20:45** — Small UX follow-up: loading a preset (via ◀/▶, the combo box, or "Import Preset...") now prefills the Save text field with that preset's own name - previously the field stayed whatever it last had (usually empty), so overwriting the preset you just loaded meant retyping its exact name by hand. Both `load_preset_at` (the named on-disk library) and the "Import Preset..." handler set `state.preset_name_input` right after a successful load, alongside the existing `selected_preset` update. `cargo build`/`cargo test --workspace` unaffected (74 tests, no logic touched - this is pure GUI state, not unit-tested, same as the rest of this file's interactive `rfd`/preset-picker flows). Built and bundled.
**Session status 2026-09-07 21:10** — The user reported the plugin now "works great" and asked about (1) building for Linux/Windows/macOS for distribution, and (2) spinning SpectralPrism out into its own standalone repo. Investigated and resolved a real licensing question along the way: nih-plug's VST3 bindings are independently GPL-3.0-licensed regardless of Steinberg's own VST3 SDK relicensing to MIT in 3.8.0 (confirmed directly against nih-plug's own README at the pinned commit), so a compiled VST3 binary is a GPL-3.0 combined work - but that only affects the *compiled binary*, not this project's own source, since `prism_dsp`/`prism_cli` don't touch nih-plug's VST3 bindings at all. Landed on: this project's own source is MIT (take/extend freely), with a clear README note that a compiled VST3 binary carries a GPL-3.0-or-later obligation from that specific external dependency, independent of this project's own license.

Executed the actual split: `git subtree split --prefix=src/plugins/SpectralPrism` in a disposable clone extracted the full 9-commit history cleanly; the result was verified to build and pass all 74 tests completely standalone before anything else happened. Copied to a permanent location, `/home/john/Desktop/00_Projects/00_John Oestmann/SpectralPrism/` (a sibling to this monorepo, confirmed working by the user), with a new LICENSE (MIT + the VST3/GPL note), README.md (features, build instructions, project structure, the license nuance, credits), and a GitHub Actions CI workflow (`.github/workflows/build.yml`, a 3-OS matrix - ubuntu/windows/macos-latest - each running `cargo xtask bundle` natively, since cross-compiling a GUI/baseview-based plugin isn't reliable) - all three new files staged but not committed, per standing instruction.

Critically, before removing anything from this monorepo: discovered the installed VST3 plugin at `00_JOM-Common/Plugins/VST3/SpectralPrism.vst3` was a symlink pointing directly into this monorepo's `src/plugins/SpectralPrism/target/bundled/` - deleting that directory would have silently broken the user's live DAW install. Built the new standalone repo's own bundle first, verified the resulting `.so` loads correctly, then re-pointed that symlink to the new location before touching the old one. Only then removed `src/plugins/SpectralPrism/` from this monorepo (tracked files via `git rm`, staged not committed; untracked leftovers - `target/`, an empty `.claude/`, two empty leftover dirs - deleted directly, since none of it was tracked history).

**No CLAP install symlink existed** (only VST3 was symlinked into a DAW's plugin folder), so nothing else needed re-pointing. **Not yet done** (needs the user, since it requires their own GitHub authentication, which isn't available in this environment - no `gh` CLI installed): create the empty `SpectralPrism` repo on GitHub (public, no initial README/license/gitignore, to avoid a conflicting initial commit), add it as `origin` in the new local repo, review/commit the staged LICENSE+README+CI files, and push. Cross-platform distribution itself (the original ask) is otherwise ready to go once that CI workflow starts running on GitHub's own Linux/Windows/macOS runners - unsigned macOS builds will still show a Gatekeeper warning until/unless notarization is set up separately later.

**This project's Kanban section is now archival** - all future SpectralPrism status tracking should happen in the standalone repo (a new board/issue tracker there, not here).
...
#### FREEZE-BUG-010 — Voices stay stuck quiet for their whole remaining sustain after a chord thins out

- **Card Title:** Voices stay stuck quiet for their whole remaining sustain after a chord thins out
- **Symptom:** Reported directly: adding voices makes each one noticeably quieter after about 4-5 (with a longer sustain setting), and they "stay fairly quiet until the number of voices active goes back to 0, then they start loud again."
- **Root cause:** `VoiceManager::active_compensation` (the polyphony gain-compensation multiplier, 1/sqrt(active voice count)) was a strict one-way ratchet - it could only ever get *stricter* (quieter) as new voices were added, and the *only* thing that ever reset it back toward 1.0 was `note_on` detecting zero currently-active voices. That was a deliberate earlier design (`FREEZE-PLAN-...`/see `active_compensation`'s own docs) to fix a real prior bug where recomputing the divisor fresh every block made an already-sounding voice audibly swell the instant an unrelated sibling voice finished - but it overcorrected: a long-held surviving note now stayed attenuated for its *entire* remaining sustain/release once its siblings released, however long that was, since literal silence was the only recovery path.
- **Implementation:** Added a second, much slower recovery path alongside the existing instant ratchet-down, in `crates/prism_dsp/src/voice.rs`. New `GAIN_RECOVERY_MS = 800.0` constant. In `process_block`, right after finished voices are pruned each block, the target compensation (`1 / sqrt(current active count)`) is recomputed; if it's *higher* than the current `active_compensation` (i.e. voices actually finished and headroom is available), `active_compensation` eases toward it via a per-block exponential approach with an ~800ms time constant - gradual enough to stay inaudible as a discrete "pump," but fast enough that a thinning texture recovers within a couple of seconds instead of waiting for total silence. The instant ratchet-down in `note_on` (the safety-critical direction, preventing a fresh chord note from clipping) is completely unchanged. `active_compensation`'s and `GAIN_COMPENSATION_SMOOTHING_MS`'s doc comments updated to describe the two-speed (fast-down, slow-up) design.
- **Assigned Agent:** Reason A
- **Card Creation Date:** 2026-09-07 20:20
- **Card Completion Note:** Superseded by `FREEZE-BUG-011` - the user tested this fix and reported the ducking was "still happening, though less." `GAIN_RECOVERY_MS` wasn't the wrong number, the whole reactive-compensation approach was the wrong tool; see `FREEZE-BUG-011` for what replaced it.
- **Process Comments:** 2026-09-07 20:20 — This is the third time this same gain-compensation mechanism needed adjustment by ear (see the two earlier bugs documented directly in `active_compensation`'s docs and this card's own regression test) - each fix so far corrected a real, measured problem but shifted the tradeoff too far in the opposite direction. 2026-09-07 20:35 — Still audible after this fix, prompting the step-back that led to `FREEZE-BUG-011`.

#### FREEZE-BUG-011 — Removed reactive polyphony gain compensation entirely (over-engineered vs. standard synth design)

- **Card Title:** Removed reactive polyphony gain compensation entirely
- **Symptom:** Direct follow-up after `FREEZE-BUG-010`'s slow-recovery fix still didn't fully resolve the ducking ("Yeap still happening, though less"). The user asked directly: "Surely there is a standard way that synths handle equal volume with multiple voices? Are we over-engineering this?"
- **Root cause / design realization:** Yes - three successive attempts (1/n, then 1/sqrt(n) with a one-way ratchet, then the ratchet plus a slow recovery) each fixed one measured problem but shifted the tradeoff, because the entire approach - dynamically changing a still-playing voice's gain in response to *other, unrelated* notes' lifecycle - is fundamentally a compressor reacting to events that have nothing to do with the note actually being heard. Any such reactive scheme will show pumping/ducking to some degree no matter how its time constants are tuned. Standard synth/instrument design doesn't do this at all: a fixed per-voice gain is used regardless of current polyphony (more notes held = more/louder sound, the same way a real piano or analog synth behaves), with a limiter as a safety net for the rare oversized chord - not a mechanism trying to keep loudness constant as voice count changes.
- **Implementation:** Removed the entire compensation system from `crates/prism_dsp/src/voice.rs`: the `active_compensation`/`smoothed_gain_compensation` fields, the `GAIN_COMPENSATION_SMOOTHING_MS`/`GAIN_RECOVERY_MS` constants, `note_on`'s ratchet-down logic, and `process_block`'s recovery-and-smoothing loop. Voices are now summed at a fixed, uncompensated gain unconditionally; `process_block`'s final stage is just `soft_limit` applied directly per sample, nothing else. `soft_limit`'s own doc comment rewritten to describe it as the *sole* safety net (a smooth, bounded tanh knee that can never hard-clip, however many voices stack - an oversized chord gets progressively more saturated-sounding instead of quieter, which is the expected/normal behavior being aimed for, not a bug to prevent).
- **Assigned Agent:** Reason A
- **Card Creation Date:** 2026-09-07 20:35
- **Card Completion Note:** Complete; `cargo build --workspace` clean with no warnings (confirms nothing else in the codebase referenced the removed fields/constants). `cargo test --workspace` passes 74 tests (61 `prism_dsp` - `gain_compensation_uses_inverse_sqrt_of_voice_count` replaced by `voices_sum_linearly_with_no_polyphony_compensation` (proves N fully-correlated voices now sum to exactly Nx a single voice, not sqrt(n)x); `gain_compensation_does_not_jump_but_recovers_gradually_when_a_voice_finishes` replaced by `surviving_voice_matches_its_solo_level_the_instant_a_sibling_finishes` (proves a surviving voice's level matches an independently-rendered solo reference immediately, with no jump or recovery period needed at all, since there's nothing to recover from anymore); 13 `prism_plugin` unchanged). Release bundle rebuilt and live at the symlinked install path. **Confirmed live 2026-09-07** - user tested and confirmed "That works so much better thanks."
- **Process Comments:** 2026-09-07 20:35 — The user's "are we over-engineering this?" question was the right call - after three tuning passes on the same reactive mechanism each traded one artifact for another, the actual fix was architectural (remove the reactive part) rather than another round of constant-tuning. Worth remembering for future DSP work here: if the second or third by-ear fix to the same mechanism is still imperfect, that's a signal to question the mechanism itself, not just its constants.

#### FREEZE-BUG-009 — Loop wraps with an audible click/glitch, worse at short Loop Length settings

- **Card Title:** Loop wraps with an audible click/glitch, worse at short Loop Length settings
- **Symptom:** Reported directly right after trying `FREEZE-PLAN-024`'s new Loop Length slider: holding a note down for a while makes the loop's own wraparound audibly click/glitch, asking for "a crossfade from end to beginning of the loop" so it can be held indefinitely without hearing the seam.
- **Root cause:** `render_frozen_loop`'s doc comment (written for `FREEZE-PLAN-024`) claimed that any hop-aligned loop length "introduces no seam discontinuity beyond what OLA itself already produces internally" - that was wrong. The frozen resynthesis is a continuously-evolving phase-vocoder texture: magnitude is fixed per bin, but each bin's phase keeps advancing hop after hop for as long as synthesis runs. The sample right before the wrap and the sample right after it are therefore two different phase-states of the same running oscillators, not a naturally periodic waveform - an integer hop count only satisfies OLA's own internal bookkeeping, it does nothing to make the wraparound itself continuous. A longer loop (the old 4-8s auto-derived range) just repeats rarely enough per second that this was easy to miss; shortening the loop makes the exact same discontinuity repeat far more often, which is what made it suddenly audible.
- **First attempt (insufficient, replaced - not layered on top of):** Added a time-domain `crossfade_loop_seam` (equal-power blend of the buffer's tail toward its own head, ~50ms). The user reported this did NOT fix it - still audibly looping/glitching. In hindsight this was the wrong tool: blending two *different phase-states of the same running oscillators* introduces its own audible comb-filtering/phasing artifact (frequency-dependent cancellation between the out-of-phase tail and head content during the blend window) - it can mask the magnitude of a discontinuity but doesn't address why the two sides of the seam differ in the first place, and can itself sound like "still looping."
- **Actual fix - phase-locking + circular OLA (a mathematically exact solution, not a blend):** Asked directly "does the whole loop need to be blended for loopability?" - no. Two changes together make the frozen resynthesis genuinely periodic instead of merely smoothed at the join: (1) new `quantize_advance_for_loop(advance, num_hops)` in `render.rs` snaps each bin's per-hop phase advance to the nearest multiple of `TAU / num_hops` - i.e. the nearest value that completes a whole number of cycles over exactly `num_hops` hops - so the oscillator bank's phase state after a full loop is identical to its state at the start (quantization step is `1/loop_duration` Hz, inaudible in practice, and inherent to how any exactly-looping wavetable/additive synth works); (2) new `ola_accumulate_circular` in `resynth.rs` wraps the last frame's overlap-add tail back around to the start of the buffer (`(pos + i) % len`) instead of dropping it, exactly as an infinite continuation of the now-periodic frame sequence would. Together the loop is mathematically periodic, not blended-to-look-continuous - `crossfade_loop_seam`/`LOOP_CROSSFADE_SECONDS` were removed entirely (replaced, not supplemented) once this landed. `render_channel`'s accumulator is now sized exactly `out_len` (no extra `FFT_SIZE` padding/truncation needed, since there's no linear tail to drop anymore).
- **Assigned Agent:** Reason A
- **Card Creation Date:** 2026-09-07 19:50
- **Card Completion Note:** Complete; `cargo build --workspace` clean, `cargo test --workspace` passes 74 tests (61 `prism_dsp` - includes a direct periodicity proof, `phase_locked_resynthesis_is_exactly_periodic_over_num_hops`, that runs the phase-locked oscillator bank for two full loops back to back and confirms the second is near-bit-identical to the first, plus tests on `quantize_advance_for_loop` and `ola_accumulate_circular`; 13 `prism_plugin` unchanged). Release bundle rebuilt and live at the symlinked install path. **Confirmed live 2026-09-07** - user tested and confirmed "Works great!"
- **Process Comments:** 2026-09-07 19:50 — The `FREEZE-PLAN-024` doc comment's "no seam discontinuity" claim was taken at face value from the existing code rather than independently verified against the resynthesis's actual phase-advance behavior - worth remembering that an existing comment describing *why* something is safe isn't the same as it actually being safe, especially once a change (shortening the loop) pushes into a regime the original author may not have tested. 2026-09-07 20:05 — First fix attempt (time-domain crossfade) didn't work; the user's follow-up question ("does the whole loop need to be blended for loopability?") was the right prompt to step back from patching the symptom and instead make the underlying oscillator model actually periodic.

#### FREEZE-BUG-008 — Shared presets can't resolve a sample path that doesn't exist on the recipient's machine

- **Card Title:** Shared presets can't resolve a sample path that doesn't exist on the recipient's machine
- **Symptom/Design concern:** Raised directly after `FREEZE-PLAN-022`: `Preset.sample_path` stores the full absolute path (e.g. `/home/john/Desktop/.../FSN-AMBIPAKT.wav`) - fine for reloading your own presets, but exposes machine-specific path structure to anyone the preset is shared with, and can never actually resolve on their machine regardless.
- **Decision:** Discussed directly (not just implemented silently, since this is a real design tradeoff) - the user chose to keep storing the full path as-is (stripping it to just a filename, or embedding the actual audio, were both considered and explicitly declined) but wanted missing-sample recovery: when a preset's referenced sample can't be found, prompt the user to locate it via a file dialog, and if they do, write the corrected path back into that preset file so it self-heals for next time on that machine.
- **Implementation:** `apply_preset` (previously `()`-returning) now returns `Option<PathBuf>` - `Some(relocated_path)` only when the original `sample_path` failed to load *and* the user successfully picked a replacement via a follow-up file dialog, `None` otherwise (preset had no sample reference, the original path resolved fine, or the user cancelled/the replacement also failed). On a load failure, an `rfd::MessageDialog` first explains what happened and which path couldn't be found, then `rfd::FileDialog` lets the user locate the real file - both call sites that apply a preset (`load_preset_at` for the named on-disk library, the "Import Preset..." button for arbitrary files) check the returned path and, if present, write the corrected preset back to wherever it came from (`save_preset(dir, name, ...)` or `write_preset_file(&path, ...)` respectively) - best-effort, a failed resave doesn't block the preset having applied correctly this time. `state.error` is left exactly as `apply_preset` set it (no longer blindly cleared to `None` afterward at either call site), so a cancelled/failed relocation still surfaces its error message instead of silently looking successful.
- **Assigned Agent:** Reason A
- **Card Creation Date:** 2026-09-07 19:10
- **Card Completion Note:** Complete; `cargo build --workspace` clean, `cargo test --workspace` passes 68 tests (this is an interactive-dialog flow - like `open_sample_dialog` and every other `rfd`-based flow in this file, it isn't unit-tested; the surrounding logic that decides *when* to prompt and what to do with the result is exercised indirectly through the existing preset-apply tests). Release bundle rebuilt and live at the symlinked install path. **Not yet confirmed live** - needs a check: create a preset referencing a sample, rename/move that sample file, then reload the preset and confirm the "sample not found" prompt appears, locating it via the dialog succeeds, and reloading the *same* preset again afterward no longer prompts (i.e. the resave actually stuck).
- **Process Comments:** 2026-09-07 19:10 — User explicitly said "I'm not sure of the fix yet" - presented as a multiple-choice tradeoff (strip to filename / embed audio / leave as-is) rather than picking one and implementing it silently; the user's chosen answer combined "leave as-is" with a specific self-healing behavior that wasn't one of the original options, which is what got built.

#### FREEZE-PLAN-023 — Build number (YYYYMMDD), saved in presets and shown in the editor

- **Card Title:** Build number (YYYYMMDD), saved in presets and shown in the editor
- **Description:** Requested directly: start stamping SpectralPrism build numbers (YYYYMMDD), include the number in preset files (so it's possible to tell which plugin version made a given preset/sound), and show it in small text at the bottom-right of the editor.
- **Implementation:** New `crates/prism_plugin/build.rs` shells out to the `date` command (`+%Y%m%d`) - no date/time crate dependency, since this project builds on Linux only, where `date` is always present - and emits `cargo:rustc-env=SPECTRALPRISM_BUILD_NUMBER=<date>`. No `cargo:rerun-if-changed` directives are set deliberately: cargo's default (rerun the build script whenever any package file changes) is exactly the wanted behavior - a fresh date on every real rebuild, not one cached from whenever `build.rs` itself was last edited. `lib.rs` reads it via `const BUILD_NUMBER: &str = env!("SPECTRALPRISM_BUILD_NUMBER")`. Added to `Preset` (`build_number: String`, `#[serde(default)]` so presets from before this existed still load, showing as an empty string rather than failing) and set in `Preset::capture()`. Displayed via a new `egui::TopBottomPanel::bottom(...)` added *before* `ResizableWindow`'s own content (egui requires side/top/bottom panels to be added before `CentralPanel`, which `ResizableWindow` uses internally, so it can shrink to leave room) - this keeps the build number genuinely fixed in the corner regardless of the main content's scroll position, rather than just being the last thing in the scrollable area. Small, dim text (`egui::RichText::small()` + `COLOR_DIM`), scaled with everything else via the existing `apply_gui_scale` mechanism.
- **Assigned Agent:** Reason A
- **Card Creation Date:** 2026-09-07 19:10
- **Card Completion Note:** Complete; verified the actual embedded string directly (`strings` on the compiled `.so` inside the bundled `.vst3`) shows today's date (`20260907`), matching `date +%Y%m%d`'s own output at build time. `cargo build --workspace` clean, `cargo test --workspace` passes 68 tests (round-trip and backward-compatibility tests updated to cover the new field; the build-time stamping itself isn't something a unit test can meaningfully check beyond "it's a non-empty string", which it already implicitly is by virtue of the crate compiling at all). Release bundle rebuilt and live at the symlinked install path. **Not yet confirmed live** - needs a look to confirm the corner text reads clearly at a few different window sizes and doesn't overlap anything.

#### FREEZE-PLAN-024 — Adjustable Loop Length param (reduce WAV export size / buffer footprint)

- **Card Title:** Adjustable Loop Length param
- **Description:** Requested directly after trying WAV export: the exported WAV "contains a lot of the same content" (the frozen loop was always 4-8s, auto-derived from the source sample's own length), asking whether a shorter loop is possible - both for the exported WAV's file size and the in-memory buffer footprint - while still looping cleanly.
- **Investigation:** Read `render::render_frozen_loop`'s own doc comment before touching anything: it already established that wrapping a hop-aligned buffer length introduces "no seam discontinuity beyond what OLA itself already produces internally" - i.e. the old 4-8s range was a *perceptual* choice (giving the evolving phase-vocoder texture time to develop before audibly repeating), not a technical requirement for click-free looping. Any hop-aligned length loops equally cleanly, so the right fix is a user-adjustable control the user can dial and judge by ear, not a smaller guessed constant.
- **Implementation:** `render_frozen_loop` now takes an explicit `loop_length_seconds: f32` argument instead of deriving it from the source sample's length - still clamped to `[MIN_LOOP_SECONDS, MAX_LOOP_SECONDS]` as a sanity bound only (widened to `0.5..8.0`, was implicitly `4.0..8.0`; `DEFAULT_LOOP_SECONDS = 4.0` preserves the old auto-derived floor as the new param's default). Threaded through `RenderRequest` (new `loop_length_seconds` field, so it participates in the existing render-throttle/dedup logic for free via `RenderRequest`'s `PartialEq`) and every call site (`Default for PrismPlugin`, `initialize()`'s synchronous first render, `process()`'s per-block comparison, `load_sample_from_path`). New `FloatParam` "Loop Length" (linear, `MIN_LOOP_SECONDS..MAX_LOOP_SECONDS`, unit " s") added to `PrismPluginParams` and to the left column of the editor UI, right under Stereo Width (the other render-time params). Added to `Preset` as `loop_length_seconds: f32` with `#[serde(default = "default_loop_length_seconds")]` (a custom default fn returning `DEFAULT_LOOP_SECONDS`, not the bare `#[serde(default)]`'s `0.0` - unlike `pan_center_pct`/`pan_width_pct` where 0.0 is a meaningful "off" value, 0.0 seconds is nonsensical here) so presets saved before this param existed still load, coming back at the length they actually used. `prism_cli` also gained a `--loop-length` flag (default `DEFAULT_LOOP_SECONDS`) for offline testing, kept distinct from its existing `--seconds` flag (which controls rendered *playback* duration, an unrelated concept).
- **Assigned Agent:** Reason A
- **Card Creation Date:** 2026-09-07 19:35
- **Card Completion Note:** Complete; `cargo build --workspace` clean, `cargo test --workspace` passes 69 tests (56 `prism_dsp`, 13 `prism_plugin` - `render.rs`'s old `loop_seconds_clamped_to_4_to_8_range` test was rewritten as `loop_length_seconds_clamped_to_min_max_range` since its premise changed from "derived from source length" to "explicit value clamped to absolute bounds", plus a new `loop_length_seconds_is_honored_within_range` test; `render_worker.rs`'s `swapping_the_source_and_re_requesting_uses_the_new_source` test was also rewritten, since it used to prove loop length scales with source length - no longer true now that length is an explicit request field - and now proves the swapped-in source's *audio content* is what's actually used instead). Release bundle rebuilt and live at the symlinked install path. **Partially confirmed live 2026-09-07** - short values now loop cleanly by ear (needed `FREEZE-BUG-009`'s phase-lock/circular-OLA fix to actually get there); the file-size-comparison half of this check is still open.
- **Process Comments:** 2026-09-07 19:35 — Deliberately investigated the existing code's own doc comments before proposing an implementation, to distinguish "is 4-8s a technical floor for seamless looping" (it isn't) from "is a shorter loop still perceptually pleasant" (a subjective question left to the user, consistent with how this user prefers to judge DSP tradeoffs by ear via sliders rather than by being told a number).
- **Process Comments:** 2026-09-07 19:10 — Requested in the same message as the parameters.txt removal and the sample-path privacy discussion.

#### FREEZE-PLAN-022 — Import/Export Preset files and Export WAV Sample

- **Card Title:** Import/Export Preset files and Export WAV Sample
- **Description:** The user's Sample Pack (`00_JOM-Common/00-MechSounds/Samples/`) only distributes sounds where a downloader can also get the project file that made them - each WAV in a category folder (e.g. `006 - Fusion Sounds/FSN-AMBIPAKT.wav`) has a same-named file in a parallel `Project Files/<category>/` folder (that particular category happens to use Strudel REPL `.txt` scripts, since that's what made those specific sounds - the convention is "same basename, a file describing how to recreate it," not a fixed format). SpectralPrism needed two things to participate: (1) Import/Export Preset via file dialogs, text-based and future-proof; (2) an Export WAV Sample button for the current frozen loop.
- **Implementation:**
  1. **Import/Export Preset**: Two new buttons next to the existing named on-disk preset library (Save/◀/▶/combo, all fixed under `presets_dir()`) - these instead go to/from any file the user picks via `rfd::FileDialog`. Reuses the *exact same* `Preset` struct/JSON format the library already uses (refactored `save_preset`/`load_preset` to delegate to new path-based `write_preset_file`/`read_preset_file`, rather than duplicating the serialize/deserialize logic) - "future-proof" here means continuing the existing `#[serde(default)]`-per-field pattern as new params get added later, not a new mechanism; a preset exported today will still partially load even if a future version adds fields this one doesn't know about (serde ignores unknown fields by default) and vice versa. A preset imported this way also updates `selected_preset` (from the file's stem) so the browser reflects what's currently loaded, same as the library-based flow.
  2. **Export WAV Sample**: New button next to "Load Sample..." (now both in one `ui.horizontal` row). Writes `loop_buffer.load()` - the actual currently-*frozen* loop, not the original source sample - via a new `write_loop_buffer_wav()`. 16-bit PCM int (not the 32-bit float `freeze_cli` itself uses for fast DSP iteration), matching 0006's own established PCM-WAV sample-export convention and for the widest compatibility with trackers/samplers (the user's stated use case: importing into OpenMPT).
- **Originally also included a `<name>-parameters.txt` sidecar** written alongside every WAV export (a human-readable listing of every param, as a manual-recreation fallback in case a future version couldn't read the JSON) - **removed the same session** once the user confirmed the JSON preset alone already carries everything needed, making the sidecar redundant. See `FREEZE-PLAN-023`/`FREEZE-BUG-008` for what replaced/followed it.
- **Assigned Agent:** Reason A
- **Card Creation Date:** 2026-09-07 18:20
- **Card Completion Note:** Complete; `cargo test --workspace` passes 68 tests (55 `prism_dsp`, 13 `prism_plugin`). `cargo build --workspace` clean with no warnings. Release bundle rebuilt and live at the symlinked install path. **Not yet confirmed live** - needs a check: export a preset, reimport it via "Import Preset...", export a WAV, and confirm OpenMPT (or another tracker) opens it cleanly.
- **Process Comments:** 2026-09-07 18:20 — Requested after the user pointed at the existing Sample Pack's `Project Files/<category>/<same-name>.txt` pairing convention as the reason these exports were needed - read that structure directly (a Strudel-script example in `006 - Fusion Sounds`) before implementing, to make sure the same-basename/sidecar-file pattern this project already uses elsewhere was matched, not just guessed at. 2026-09-07 19:10 — Parameters-sidecar half scrapped per direct feedback; see `FREEZE-PLAN-023`.
