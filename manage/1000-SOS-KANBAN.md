...
**Board Last Updated:** 2026-09-07 00:10 by Reason A  
**Session status 2026-09-07 00:10** — Another follow-up round after trying `FREEZE-PLAN-013`/`FREEZE-PLAN-014` live: the 150% scale still wasn't enough and the 620×560 window guess was wrong in both dimensions (too narrow, too tall) - both bumped further (`FREEZE-PLAN-015`). The user also found a genuinely separate, real bug while testing sample loading: a freshly loaded sample doesn't actually take over playback until Freeze Point is nudged (`FREEZE-BUG-006`) - root-caused to a host-timing race between `editor()` and `initialize()`, not anything related to the render pipeline itself. Also shrank the ADSR graph to match the waveform graph's height (`FREEZE-PLAN-016`), a small polish item bundled into the same session. All three implemented, built, and bundled; `cargo test --workspace` passes 59 tests (50 `prism_dsp`, 9 `prism_plugin` - one new regression test for the sample-loading race). **Not yet confirmed live** - needs a look in Bespoke Synth covering: no more clipped/cut-off elements at the new default size, the two graphs lining up, and specifically loading a sample *without* touching Freeze Point afterward and confirming it's audibly the new sample, not the old frozen content.
